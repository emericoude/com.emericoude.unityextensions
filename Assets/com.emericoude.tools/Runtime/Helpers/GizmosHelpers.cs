...
namespace Emericoude.Helpers
{
	public static class GizmosHelpers
	{
		...
		public static void DrawWireCapsule (CapsuleCollider capsuleCollider)
		{
			...
			GizmosHelpers.DrawWireCapsule(point0, point1, radius);
		}
		...
		public static void DrawWireCapsule (Vector3 point0, Vector3 point1, float radius)
		{
			...
			GizmosHelpers.DrawWireCylinder(center, orientation, radius, height, 16);
		...
		{
			capsuleCollider.GetParametersForOverlap(out Vector3 point0, out Vector3 point1, out float radius);
			GizmosHelpers.DrawCapsule(point0, point1, radius, cylinderResolution);
		}
		...
		public static void DrawCapsule (Vector3 point0, Vector3 point1, float radius, int cylinderResolution = 128)
		{
			...
			GizmosHelpers.DrawCylinder(center, orientation, radius, length, cylinderResolution);
		}
		...
		public static void DrawWireCylinder (Vector3 position, Vector3 orientation, float radius, float height, int resolution = 128)
		{
			...
			UnityEngine.Gizmos.color = oldColor.WithAlpha(oldColor.a / 2); //half the alpha so transparency works on line overlaps
		...
		public static void DrawCollider (Collider collider)
		{
			...
			{
				GizmosHelpers.DrawCapsule(capsuleCollider);
			}
		...
		public static void DrawWireCollider (Collider collider)
		{
			...
			{
				GizmosHelpers.DrawWireCapsule(capsuleCollider);
			}
			else if (collider is MeshCollider meshCollider)
			{
		...
		public static void DrawSphereCast (Vector3 position, float radius, Vector3 direction, out RaycastHit hit, float maxDistance = Mathf.Infinity, int layerMask = ~0, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal, float animationSpeed = 1.0f)
		{
			...
			GizmosHelpers.DrawCapsule(position, endPoint, radius);
		...
		public static void DrawWireSphereCast (Vector3 position, float radius, Vector3 direction, out RaycastHit hit, float maxDistance = Mathf.Infinity, int layerMask = ~0, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal, float animationSpeed = 1.0f)
		{
			...
			GizmosHelpers.DrawWireCapsule(position, endPoint, radius);
			if (AnimatedGizmos.Enabled)
			{
				//float animationDuration = Vector3.Distance(position, endPoint) * (animationSpeed / 100.0f); //speed accurate
				float animationDuration = 1.0f / Vector3.Distance(position, endPoint) / animationSpeed; //distance-based
